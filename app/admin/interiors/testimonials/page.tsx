...
export default function InteriorTestimonialsPage() {
  ...
  const [formData, setFormData] = useState<Partial<Testimonial>>({ name: "", location: "", text: "", image: "" });
  ...
  const loadTestimonials = async () => {
    setLoading(true);
    try {
      const data = await testimonialService.getTestimonialList(true);
          ...
          <div className="w-12 h-12 rounded-xl overflow-hidden shadow-md border border-charcoal/5 flex-shrink-0">
            {item.image ? (
              ...
              <div className="w-full h-full bg-charcoal/5 flex items-center justify-center">
                <User size={16} className="text-charcoal/30" />
              </div>
            )}
          </div>
          <div>
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingTestimonial) {
        await testimonialService.updateTestimonial(editingTestimonial.id, formData, selectedFile || undefined);
      ...
      showError(editingTestimonial ? "Failed to update testimonial" : "Failed to create testimonial");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6 pb-12">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-semibold text-charcoal">Testimonials</h1>
            <p className="text-xs text-charcoal/40 mt-0.5">Manage client testimonials</p>
          </div>
        </div>
      </div>
